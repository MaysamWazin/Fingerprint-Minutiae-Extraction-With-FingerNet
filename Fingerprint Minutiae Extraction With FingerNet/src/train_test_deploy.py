...
batch_size = 2
use_multiprocessing = False
train_set = [r'C:\Users\kubra\Desktop\FingerNet-masterr (1)\FingerNet-master\datasets\CISL24218']
test_set = [r'C:\Users\kubra\Desktop\FingerNet-masterr (1)\FingerNet-master\datasets\CISL24218']
train_sample_rate = [1]

deploy_set = [
    r'C:\Users\kubra\Desktop\FingerNet-masterr (1)\FingerNet-master\datasets\NISTSD27\images',
    r'C:\Users\kubra\Desktop\FingerNet-masterr (1)\FingerNet-master\datasets\CISL24218',
    r'C:\Users\kubra\Desktop\FingerNet-masterr (1)\FingerNet-master\datasets\FVC2002DB2A',
    r'C:\Users\kubra\Desktop\FingerNet-masterr (1)\FingerNet-master\datasets\NIST4',
    r'C:\Users\kubra\Desktop\FingerNet-masterr (1)\FingerNet-master\datasets\NIST14'
]

pretrain = r'C:\Users\kubra\Desktop\FingerNet-masterr (1)\FingerNet-master\models\released_version\Model.model'
...
def deploy(deploy_set, set_name=None):
    ...
    if len(img_name) == 0:
        deploy_set = os.path.join(deploy_set, 'images') + os.sep
        _, img_name = get_files_in_folder(deploy_set, '.bmp')
    first_img_path = os.path.join(deploy_set, img_name[0] + '.bmp')
    img_size = imageio.imread(first_img_path).shape
    ...
    for i in range(0,len(img_name)):
        ...
        time_start = time()    
        image_path = os.path.join(deploy_set, img_name[i] + '.bmp')
        ...
        enh_vis = np.squeeze(enhance_img) * ndimage.zoom(np.round(np.squeeze(seg_out)), [8,8], order=0)
        enh_vis = np.nan_to_num(enh_vis)
        enh_vis = ((enh_vis + np.pi) / (2 * np.pi) * 255.0).clip(0, 255).astype(np.uint8)
        seg_vis = ndimage.zoom(np.round(np.squeeze(seg_out)), [8,8], order=0)
        seg_vis = (seg_vis.clip(0, 1) * 255).astype(np.uint8)
        imageio.imwrite("%s/%s/%s_enh.png"%(output_dir, set_name, img_name[i]), enh_vis)
        imageio.imwrite("%s/%s/%s_seg.png"%(output_dir, set_name, img_name[i]), seg_vis) 
